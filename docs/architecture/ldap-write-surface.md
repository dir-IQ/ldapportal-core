...
| `SchemaLdifService` | `com.ldapportal.ldap` | Superadmin schema updates via LDIF: applies `attributeTypes`/`objectClasses` change records to the schema container after a mandatory preview. Scope-guarded to the schema subentry (`cn=schema` / `cn=schema,cn=config`). OpenLDAP writes go through a caller-supplied `cn=config` bind (`openUnboundConnection` + bind); OpenDJ writes go through `withConnectionUnreplicated`. | No — schema is not data; both paths are deliberately uncaptured. |
