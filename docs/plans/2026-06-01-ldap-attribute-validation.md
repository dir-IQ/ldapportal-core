...
- **Date:** 2026-06-01
- **Status:** Not started (planning, 2026-06-01).
...
1. `DnValidator` + `@Dn`/`@ValidRdn` constraints + DTO annotations + tests;
   refactor `createContainer` onto it. *(Highest value, lowest blast
   radius.)*
2. `LdapAttributeValidator` (wraps `validateAttributes` + syntax + schema
   `MUST`) wired into admin create/update + tests.
3. `editable`/`hidden` server enforcement.
4. Frontend shared util + `FormField` error state + wire admin forms +
   tests.
5. Dedupe self-service onto the shared util.
6. Operator doc note + flip this **Status** to *In progress* → *Shipped*.
