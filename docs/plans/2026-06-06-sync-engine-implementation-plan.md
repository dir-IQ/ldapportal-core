...
### Phase 0 — clean baseline (one coherent PR) — **Shipped (2026-06-06)**
...
**As shipped:** the baseline was regenerated by applying V1..V22 to a scratch
Postgres, dropping the five `replication_*`/`reconciliation_*` tables and
`directory_connections.replication_enabled`, creating `sync_links` / `sync_set`
/ `membership` / `recompute_request`, and `pg_dump`-ing the result into a single
`V1__baseline.sql` (V2..V22 deleted). The whole `com.ldapportal.ldap.replication`
package (incl. `reconcile`), its entities/enums/repos/services/controllers/DTOs,
and all their tests were removed; `LdapConnectionFactory` lost its capture
wrapper (the `withConnectionUnreplicated` seam is retained for Phase 1) and
`ActivityDashboardService` dropped its replication/reconcile queries. The
shared `ldap/changelog/*` strategies stay (audit ingestion uses them);
`ReplicationOperationType` was renamed `LdapChangeOp`. New `sync_*` JPA entities
(+ `@IdClass` composite keys for `membership`/`recompute_request`), their repos,
and the `IdentityStrategy` SPI (entryUUID / objectGUID / Entra skeletons +
registry) landed. Boot verified under Hibernate `validate` against the
regenerated baseline on Postgres 16; core (950) + frontend (375) + isva suites
pass. **Deferred to later phases:** the `DirectorySyncView` UI + `api/replication`
client (Phase 2 owns the UI), `sync.*` audit actions, and rewriting the
changelog-strategy javadoc that still names removed classes (Phase 3 repurposes
that SPI).
