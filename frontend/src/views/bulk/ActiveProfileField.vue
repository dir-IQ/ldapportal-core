...
<!--
  ...
  picker, so this just reflects that choice: a bordered box whose border + text
  take the profile's theme colour, with the target DN beneath. Falls back to
  neutral styling when the profile has no theme colour, and to an em-dash when
  no profile is selected.
-->
<template>
  <div>
    <label class="block text-sm font-medium text-gray-700 mb-1">Active profile</label>
    <div class="flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold bg-white"
         :style="color ? { borderColor: color, color } : {}"
         :class="color ? '' : 'border-gray-300 text-gray-900'">
